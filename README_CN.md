...
*   **🧠 DeepSeek 推理核心**: 采用“推理-编码”模式。智能体在采取行动前会先通过 `<think>` 进行深度思考，处理复杂任务时具有更高的准确性。
    *   **实时思考过程**: 在右侧“运行记录”面板实时展示 AI 的思维链，主对话区保持整洁，只展示最终回复。
*   **🔌 模块化技能中心**: 
    *   **可视化管理**: 内置“功能中心”面板，直观展示已安装的技能。
    *   **分类展示**: 自动区分“标准功能模块”和“AI 生成的技能”，方便管理。
    *   **动态扩展**: 智能体可以将通用的算法逻辑固化为新技能，不断进化。
*   **🛡️ 安全执行**:
...
*   **`core/`**:
    *   `agent.py`: 管理 LLM 交互、System Prompt 策略（包含技能创建限制）和对话历史。
    *   `skill_manager.py`: 处理工具的动态加载、元数据解析（支持中文描述）和分类管理。
    *   `config_manager.py`: 持久化用户设置。
*   **`skills/`**:
    *   扩展功能的插件。每个技能都有自己的 `impl.py` 和 `SKILL.md`（支持 `description_cn`）。
*   **`main.py`**: PySide6 GUI 入口点，包含主窗口、技能中心对话框（Tab 分页）、聊天气泡组件等。
